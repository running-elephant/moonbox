    ...
    return new AbstractEnumerable<Object>() {
      public Enumerator<Object> enumerator() {
        SearchResponse searchResponse = client.prepareSearch(dbName).setTypes(typeName)
                .setSource(queryString).execute().actionGet();
        if (searchResponse.getAggregations() != null) {
          //final Function1<List<Object>, Object> getter = ElasticsearchAggregationEnumerator.getter(fields);
			Aggregations aggregations = searchResponse.getAggregations();
			return new ElasticsearchAggregationEnumerator(aggregations, fields);
        } else {
