...
private[kafka010] class KafkaSourceProvider extends DataSourceRegister
  ...
  override def createSink(
    ...
    val umsParams = parameters.filterKeys(!_.toLowerCase(Locale.ROOT).startsWith("kafka."))
    new KafkaSink(sqlContext, new ju.HashMap[String, String](specifiedKafkaParams.asJava),
      new ju.HashMap[String, String](umsParams.asJava), defaultTopic)
  }
  ...
  override def createRelation(
    ...
    KafkaWriter.write(outerSQLContext.sparkSession, data.queryExecution,
      new ju.HashMap[String, String](specifiedKafkaParams.asJava),
      new ju.HashMap[String, String](umsParams.asJava), topic)
    ...
    if (caseInsensitiveParams.contains(s"kafka.${ConsumerConfig.KEY_DESERIALIZER_CLASS_CONFIG}")) {
      throw new IllegalArgumentException(
        s"Kafka option '${ConsumerConfig.KEY_DESERIALIZER_CLASS_CONFIG}' is not supported as keys "
          + "are deserialized as string with StringDeserializer. Use DataFrame operations "
    ...
    if (caseInsensitiveParams.contains(s"kafka.${ConsumerConfig.VALUE_DESERIALIZER_CLASS_CONFIG}")) {
      throw new IllegalArgumentException(
        s"Kafka option '${ConsumerConfig.VALUE_DESERIALIZER_CLASS_CONFIG}' is not supported as "
          + "value are deserialized as string with StringDeserializer. Use DataFrame "
