...
private[kafka010] class KafkaRelation(
  ...
  override def schema: StructType = userSchema.getOrElse(KafkaOffsetReader.kafkaSchema)
  ...
  override def buildScan(): RDD[Row] = {
    ...
    val rdd = new KafkaSourceRDD(
      ...
      InternalRow(
        UTF8String.fromString(cr.key),
        UTF8String.fromString(cr.value),
  ...
  override def insert(data: DataFrame, overwrite: Boolean): Unit = {
    ...
    data.write.format("kafka").options(sourceOptions).mode(mode).save()
  }
}
