...
private[kafka010] class KafkaSourceRDD(
  ...
  extends RDD[ConsumerRecord[String, String]](sc, Nil) {
  ...
  override def take(num: Int): Array[ConsumerRecord[String, String]] = {
    val nonEmptyPartitions =
      this.partitions.map(_.asInstanceOf[KafkaSourceRDDPartition]).filter(_.offsetRange.size > 0)

    if (num < 1 || nonEmptyPartitions.isEmpty) {
      return new Array[ConsumerRecord[String, String]](0)
    }
    ...
    val buf = new ArrayBuffer[ConsumerRecord[String, String]]
    val res = context.runJob(
      this,
      (tc: TaskContext, it: Iterator[ConsumerRecord[String, String]]) =>
  ...
  override def compute(
      thePart: Partition,
      context: TaskContext): Iterator[ConsumerRecord[String, String]] = {
    ...
    if (range.fromOffset == range.untilOffset) {
      ...
      val underlying = new NextIterator[ConsumerRecord[String, String]]() {
        var requestOffset = range.fromOffset

        override def getNext(): ConsumerRecord[String, String] = {
