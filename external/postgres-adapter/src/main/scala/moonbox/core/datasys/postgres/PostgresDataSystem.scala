...
package moonbox.core.datasys.postgres
...
import org.apache.spark.sql.sqlbuilder.{MbPostgresDialect, MbSqlBuilder}
import org.apache.spark.sql.types._
import org.apache.spark.sql.{DataFrame, Row, SaveMode, SparkSession}

import scala.collection.mutable.ArrayBuffer

class PostgresDataSystem(props: Map[String, String])
	...
	override def tableNames(): Seq[String] = {
		val showTablesSql = """SELECT table_name FROM information_schema.tables where table_schema = 'public'"""
	...
	override def buildQuery(plan: LogicalPlan, sparkSession: SparkSession): DataTable = {
		val sqlBuilder = new MbSqlBuilder(plan, new MbPostgresDialect)
		...
		val resultSet = try {
			...
			case e: Exception =>
				conn.close()
				logWarning("postgres pushdown execute failed.", e)
				throw e
		}
	...
	override def fastEquals(other: DataSystem): Boolean = {
		other match {
			case oracle: PostgresDataSystem =>
	...
	override def test(): Unit = {
		...
		try {
			...
			case e: Exception =>
				logError("postgres test failed.", e)
				throw e
		} finally {
			if (connection != null) {
				connection.close()
