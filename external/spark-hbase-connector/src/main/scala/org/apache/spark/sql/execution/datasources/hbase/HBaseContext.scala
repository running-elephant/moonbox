...
import org.apache.hadoop.hbase.io.hfile.{CacheConfig, HFile, HFileContextBuilder, HFileWriterV3}
...
import org.apache.spark.sql.execution.datasources.hbase.HBaseRDDFunctions._
...
class HBaseContext(@transient val sc: SparkContext,
  ...
  Unit = {
    ...
    val hfileCompression = Compression.Algorithm.valueOf(defaultCompressionStr)
  ...
  Unit = {
    ...
    val defaultCompressionStr = config.get("hfile.compression",
      Compression.Algorithm.NONE.getName)
    val defaultCompression = Compression.Algorithm.valueOf(defaultCompressionStr)
    val nowTimeStamp = System.currentTimeMillis()
    val tableRawName = tableName.getName
