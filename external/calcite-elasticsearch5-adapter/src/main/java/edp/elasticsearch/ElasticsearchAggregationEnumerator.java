...
public class ElasticsearchAggregationEnumerator implements Enumerator<Object> {
	...
	public ElasticsearchAggregationEnumerator(Aggregations aggregations, List<Map.Entry<String, Class>> fields, Integer fetchLines) {
		...
		try {
			handleAggregations(this.aggregations, this.lines, 0);
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	private void handleAggregations(Aggregations aggregations, List<Object[]> lines, int level) throws Exception {
		...
		if (aggregation instanceof SingleBucketAggregation) {
			...
			handleAggregations(singleBucktAggs, lines, level + 1);
			return;
		}
		...
		if (aggregation instanceof MultiBucketsAggregation) {
			...
			for(MultiBucketsAggregation.Bucket bucket : buckets) {
				...
				handleAggregations(subAgg, lines, level +1 );
	...
	public boolean moveNext() {
		...
		if (cursor.hasNext()) {
			...
			//From ES5 official website: If the request specifies aggregations, only the initial search response will contain the aggregations results.
			current = null;
			return false;
		}
	}
