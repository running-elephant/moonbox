...
import edp.moonbox.calcite.rel.core.Filter;
import edp.moonbox.calcite.rel.metadata.RelMetadataQuery;
import edp.moonbox.calcite.rex.RexCall;
import edp.moonbox.calcite.rex.RexInputRef;
import edp.moonbox.calcite.rex.RexLiteral;
import edp.moonbox.calcite.rex.RexNode;
import edp.moonbox.calcite.util.JsonBuilder;
...
public class ElasticsearchFilter extends Filter implements ElasticsearchRel {
  ...
  static class Translator {
    ...
    final Map<String, String> nullMap = new LinkedHashMap<>();
    ...
    private String translateMatch(RexNode condition) {
      ...
      String str = "\"query\" : " + builder.toJsonString(map).replaceAll("\\s+", "");
      return str;
    }
    ...
    private Object translateOr(RexNode condition) {
      ...
      for (RexNode node : orNodes) {
        ...
        if (andNodes.size() > 0) {
          ...
          //System.out.println("must " + andNodes.toString());
      ...
      if (orNodes.size() > 1) {
        ...
        //System.out.println("should " + list.toString());
        // boolean filters
        LinkedHashMap<String, Object> filterEvaluator = new LinkedHashMap<>();
    ...
    private List<Map<String, Object>> translateAnd(RexNode node0) {
      ...
      for (Map.Entry<String, RexLiteral> entry : eqMap.entrySet()) {
        ...
        //System.out.println("term " + filter.toString());
      ...
      for (Map.Entry<String, Collection<Pair<String, RexLiteral>>> entry : multimap.asMap().entrySet()) {
        ...
        for (Pair<String, RexLiteral> s : entry.getValue()) {
          if(s.left.equals("not")) {
            ...
            //System.out.println("not term " + map2.toString());

            map.put("not", termMap);
          } else if(s.left.equals("wildcard")) {

            //addPredicate(map2, s.left, literalValue(s.right));
            Map<String, Object> filter = new HashMap<>();
            String likeQuery = literalValue(s.right).toString().replace('%', '*').replace('_', '?');
            filter.put(entry.getKey(), likeQuery);

            //System.out.println("wildcard " + likeQuery);

            map.put("wildcard", filter);
          } else {
            addPredicate(map2, s.left, literalValue(s.right));
            ...
            //System.out.println("range " + filter.toString());
    ...
    private Void translateMatch2(RexNode node) {
      ...
      case LIKE:
        return translateBinary("wildcard", "wildcard", (RexCall) node);
      default:
    ...
    private boolean translateBinary2(String op, RexNode left, RexNode right) {
      ...
      // where 1 = 0
      case LITERAL:
        RexLiteral left0 = (RexLiteral) left;
        String leftName = left0.getValue().toString();
        translateOp2(op, leftName, rightLiteral);
        return true;
      //
      case INPUT_REF:
