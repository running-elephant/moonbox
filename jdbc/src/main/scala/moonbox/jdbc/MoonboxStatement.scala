...
class MoonboxStatement(connection: MoonboxConnection) extends Statement {
  ...
  override def execute(sql: String, autoGeneratedKeys: Int) = execute(sql)

  // TODO:
  override def execute(sql: String, columnIndexes: scala.Array[Int]) = execute(sql)

  // TODO:
  override def execute(sql: String, columnNames: scala.Array[String]) = execute(sql)
