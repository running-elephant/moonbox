...
class Main(
  ...
  override def handleMessage: Receive = {
    ...
    case schema@SchemaRequest(org, username, sqls, database, analyzable) =>
      val requester = sender()
      Future {
        val servicer = new Servicer(org, username, database, conf, self)
        servicer.schema(sqls, analyzable)
      }.onComplete {
        case Success(response) =>
          requester ! response
        case Failure(e) =>
    ...
    case lineage@LineageRequest(org, username, sqls, database, analyzable) =>
      val requester = sender()
      Future {
        val servicer = new Servicer(org, username, database, conf, self)
        servicer.lineage(sqls, analyzable)
      }.onComplete {
        case Success(response) =>
          requester ! response
        case Failure(e) =>
