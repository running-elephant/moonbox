...
case class ArrayMap(left: Expression, right: Expression) extends BinaryExpression with CodegenFallback {
    ...
    override def nullSafeEval(array: Any, lambda: Any): Any = {
        ...
        if (elementType != NullType) {
            val convertedLambda = ExprUtil.genArrayMapCode(right, elementType)

            val newData: Array[AnyRef] = JaninoCodeGen.map(data, elementType, convertedLambda)
    ...
    override def prettyName: String = "array_map"
}
