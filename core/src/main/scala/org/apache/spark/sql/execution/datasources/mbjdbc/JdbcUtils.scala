...
object JdbcUtils extends Logging {
	...
	/**
	  * Returns an Insert SQL statement for inserting a row into the target table via JDBC conn.
	  */
	def getInsertStatementWithSequence(
		                      table: String,
		                      rddSchema: StructType,
		                      tableSchema: Option[StructType],
		                      isCaseSensitive: Boolean,
		                      dialect: JdbcDialect,
		                      colToSequence: Map[String, String]): String = {
		val columns = if (tableSchema.isEmpty) {
			rddSchema.fields.map(x => dialect.quoteIdentifier(x.name)).mkString(",")
		} else {
			val columnNameEquality = if (isCaseSensitive) {
		...
		val placeholders = rddSchema.fields.map { f =>
			colToSequence.get(f.name).map(s => s"$s.nextval").getOrElse("?")
		}.mkString(",")
		s"INSERT INTO $table ($columns) VALUES ($placeholders)"
	}
	...
	private type JDBCValueSetter = (PreparedStatement, Int, Row, Int) => Unit
	...
	private def makeSetter(
		...
		case IntegerType =>
			(stmt: PreparedStatement, stmtPos: Int, row: Row, pos: Int) =>
				stmt.setInt(stmtPos, row.getInt(pos))

		case LongType =>
			(stmt: PreparedStatement, stmtPos: Int, row: Row, pos: Int) =>
				stmt.setLong(stmtPos, row.getLong(pos))

		case DoubleType =>
			(stmt: PreparedStatement, stmtPos: Int, row: Row, pos: Int) =>
				stmt.setDouble(stmtPos, row.getDouble(pos))

		case FloatType =>
			(stmt: PreparedStatement, stmtPos: Int, row: Row, pos: Int) =>
				stmt.setFloat(stmtPos, row.getFloat(pos))

		case ShortType =>
			(stmt: PreparedStatement, stmtPos: Int, row: Row, pos: Int) =>
				stmt.setInt(stmtPos, row.getShort(pos))

		case ByteType =>
			(stmt: PreparedStatement, stmtPos: Int, row: Row, pos: Int) =>
				stmt.setInt(stmtPos, row.getByte(pos))

		case BooleanType =>
			(stmt: PreparedStatement, stmtPos: Int, row: Row, pos: Int) =>
				stmt.setBoolean(stmtPos, row.getBoolean(pos))

		case StringType =>
			(stmt: PreparedStatement, stmtPos: Int, row: Row, pos: Int) =>
				stmt.setString(stmtPos, row.getString(pos))

		case BinaryType =>
			(stmt: PreparedStatement, stmtPos: Int, row: Row, pos: Int) =>
				stmt.setBytes(stmtPos, row.getAs[Array[Byte]](pos))

		case TimestampType =>
			(stmt: PreparedStatement, stmtPos: Int, row: Row, pos: Int) =>
				stmt.setTimestamp(stmtPos, row.getAs[java.sql.Timestamp](pos))

		case DateType =>
			(stmt: PreparedStatement, stmtPos: Int, row: Row, pos: Int) =>
				stmt.setDate(stmtPos, row.getAs[java.sql.Date](pos))

		case t: DecimalType =>
			(stmt: PreparedStatement, stmtPos: Int, row: Row, pos: Int) =>
				stmt.setBigDecimal(stmtPos, row.getDecimal(pos))
		...
		case _ =>
			(_: PreparedStatement, stmtPos: Int, _: Row, pos: Int) =>
	...
	def savePartition(
		                 ...
		                 dialect: JdbcDialect,
		                 isolationLevel: Int,
		                 colToSequence: Map[String, String]
	                 ): Iterator[Byte] = {
		...
		try {
			...
			try {
				...
				if (colToSequence.isEmpty) {
					while (iterator.hasNext) {
						...
						while (i < numFields) {
							if (row.isNullAt(i)) {
								...
								setters(i).apply(stmt, i + 1, row, i)
							}
							i = i + 1
						}
						stmt.addBatch()
					...
					var stmtPos = 0
					val rowPosToStmtPos = rddSchema.fields.zipWithIndex.map { case (f, idx) =>
						if (!colToSequence.contains(f.name)) stmtPos = stmtPos + 1
						(idx, stmtPos)
					}.toMap
					while (iterator.hasNext) {
						val row = iterator.next()
						var i = 0
						while (i < numFields) {
							if (!colToSequence.contains(rddSchema.fields(i).name)) {
								if (row.isNullAt(i)) {
									stmt.setNull(rowPosToStmtPos(i), nullTypes(i))
								} else {
									setters(i).apply(stmt, rowPosToStmtPos(i), row, i)
								}
							}
							i = i + 1
						}
	...
	def saveTable(
		...
		val colToSequence = if (url.startsWith("jdbc:oracle")) {
			val seqColumnsString = properties.getProperty("sequence.columns")
			val seqNamesString = properties.getProperty("sequence.names")
			if (seqColumnsString != null && seqNamesString != null) {
				val seqColumns = seqColumnsString.split(",").map(_.trim)
				val seqNames = seqNamesString.split(",").map(_.trim)
				if (seqColumns.length == 0 || seqNames.length == 0) {
					Map[String, String]()
				} else {
					CaseInsensitiveMap.apply(seqColumns.zip(seqNames).toMap[String, String])
				}
			} else {
				Map[String, String]()
			}
		...
		val insertStmt = if (colToSequence.isEmpty) {
			getInsertStatement(table, rddSchema, tableSchema, isCaseSensitive, dialect)
		} else {
			getInsertStatementWithSequence(table, rddSchema, tableSchema, isCaseSensitive, dialect, colToSequence)
		}
		...
		repartitionedDF.foreachPartition(iterator => savePartition(
			getConnection, table, iterator, rddSchema, insertStmt, batchSize, dialect, isolationLevel, colToSequence)
		)
	}
