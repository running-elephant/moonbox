...
case class InsertIntoHiveTable(
	...
	override def run(sparkSession: SparkSession): Seq[Row] = {
		...
		// the real hive table
		val identifier = table.properties.get("hivetable").map { t =>
			table.identifier.copy(table = t, database = table.properties.get("hivedb"))
		}.getOrElse(table.identifier.copy(database = table.properties.get("hivedb")))

		val hiveCatalogTable = table.copy(identifier = identifier)
		val client = HiveClientUtils.getHiveClient(hiveCatalogTable.properties)
		...
		val hiveQlTable = HiveClientImpl.toHiveTable(hiveCatalogTable)
		...
		if (partitionColumnNames.toSet != partition.keySet) {
			throw new SparkException(
				s"""Requested partitioning does not match the ${hiveCatalogTable.identifier.table} table:
				   |Requested partitions: ${partition.keys.mkString(",")}
				   |Table partitions: ${hiveCatalogTable.partitionColumnNames.mkString(",")}""".stripMargin)
		}
		...
		if (partition.nonEmpty) {
			if (numDynamicPartitions > 0) {
				...
				def loadDynamicPartitions(
					                         ...
					                         replace: Boolean,
					                         numDP: Int): Unit = {
					if (!client.tableExists(db, table)) {
						throw new NoSuchTableException(db = db, table = table)
					}

					val orderedPartitionSpec = new util.LinkedHashMap[String, String]()
					client.getTable(db, table).partitionColumnNames.foreach { colName =>
				...
				loadDynamicPartitions(
					db = hiveCatalogTable.database,
					table = hiveCatalogTable.identifier.table,
					tmpLocation.toString,
					partitionSpec,
					overwrite,
					numDynamicPartitions)

				// register created partitions to session catalog
				val createdPartitions = client.getPartitions(hiveCatalogTable, None)

				val partitions = sparkSession.sessionState.catalog.listPartitions(table.identifier)

				 sparkSession.sessionState.catalog.createPartitions(table.identifier, createdPartitions.diff(partitions), ignoreIfExists = true)
			} else {
				// scalastyle:off
				// ifNotExists is only valid with static partition, refer to
				// https://cwiki.apache.org/confluence/display/Hive/LanguageManual+DML#LanguageManualDML-InsertingdataintoHiveTablesfromqueries
				// scalastyle:on
				def lowerCasePartitionSpec(spec: TablePartitionSpec): TablePartitionSpec = {
					spec.map { case (k, v) => k.toLowerCase -> v }
				}

				def restorePartitionSpec(
				...
				val oldPart =
				client.getPartitionOption(hiveCatalogTable.database, hiveCatalogTable.identifier.table, lowerCasePartitionSpec(partitionSpec)).map { part =>
					part.copy(spec = restorePartitionSpec(part.spec, client.getTable(hiveCatalogTable.database, hiveCatalogTable.identifier.table).partitionColumnNames))
				}
				/*externalCatalog.getPartitionOption(
					hiveCatalogTable.database,
					hiveCatalogTable.identifier.table,
					partitionSpec)*/
				...
				if (oldPart.isEmpty || !ifPartitionNotExists) {
					...
					val createdPartition =
						client.getPartitionOption(hiveCatalogTable.database, hiveCatalogTable.identifier.table, lowerCasePartitionSpec(partitionSpec)).map { part =>
							part.copy(spec = restorePartitionSpec(part.spec, client.getTable(hiveCatalogTable.database, hiveCatalogTable.identifier.table).partitionColumnNames))
						}
					// register new created partition to session catalog
					sparkSession.sessionState.catalog.createPartitions(table.identifier, createdPartition.map(Seq(_)).getOrElse(Seq()), ignoreIfExists = true)
				}
			}
		} else {

			if (!client.tableExists(hiveCatalogTable.database,
				hiveCatalogTable.identifier.table)) {
				throw new NoSuchTableException(hiveCatalogTable.database, table = hiveCatalogTable.identifier.table)
			}

			client.loadTable(
				tmpLocation.toString,
				s"${hiveCatalogTable.database}.${hiveCatalogTable.identifier.table}",
				overwrite,
				isSrcLocal = false)
