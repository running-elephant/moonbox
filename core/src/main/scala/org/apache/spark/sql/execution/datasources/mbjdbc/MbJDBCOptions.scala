...
class MbJDBCOptions(
  ...
  import MbJDBCOptions._

  def this(parameters: Map[String, String]) = this(CaseInsensitiveMap(parameters))

  def this(url: String, table: String, parameters: Map[String, String]) = {
    this(CaseInsensitiveMap(parameters ++ Map(
      MbJDBCOptions.JDBC_URL -> url,
      MbJDBCOptions.JDBC_TABLE_NAME -> table)))
  }
  ...
  var lowerBound = parameters.get(JDBC_LOWER_BOUND)
  // the upper bound of the partition column
  var upperBound = parameters.get(JDBC_UPPER_BOUND)

  val autoComputePartitionBound = parameters.getOrElse(JDBC_AUTO_COMPUTE_PARTITION_BOUND, "false").toBoolean
  ...
  require((partitionColumn.isEmpty && lowerBound.isEmpty && upperBound.isEmpty) ||
    ...
    (partitionColumn.isDefined && numPartitions.isDefined && autoComputePartitionBound),
    s"When reading JDBC data sources, users need to specify all or none of the following " +
      s"options: '$JDBC_PARTITION_COLUMN', '$JDBC_LOWER_BOUND', '$JDBC_UPPER_BOUND', " +
      s"'$JDBC_AUTO_COMPUTE_PARTITION_BOUND' and '$JDBC_NUM_PARTITIONS'")
  ...
  def setLowerBound(lowerBound: String) = this.lowerBound = Some(lowerBound)

  def setUpperBound(upperBound: String) = this.upperBound = Some(upperBound)
}

class MbJdbcOptionsInWrite(
                            @transient override val parameters: CaseInsensitiveMap[String])
  extends MbJDBCOptions(parameters) {
...
object MbJDBCOptions {
  ...
  val JDBC_AUTO_COMPUTE_PARTITION_BOUND = newOption("autoComputePartitionBound")
