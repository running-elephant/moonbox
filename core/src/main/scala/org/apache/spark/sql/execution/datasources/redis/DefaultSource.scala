...
case class RedisRelation(props: Map[String, String], userSchema: StructType)
	(@transient val sQLContext: SQLContext) extends BaseRelation with InsertableRelation {
	...
	override def insert(data: DataFrame, overwrite: Boolean): Unit = {
		redisClient.put("SCHEMA", jobId, data.schema.json)
		data.foreachPartition { partition =>
			val redis = new RedisCache(servers)
			redis.bulkPut(jobId, partition)
		}
	}
}
