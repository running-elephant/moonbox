...
class SQLBuilder(logicalPlan: LogicalPlan, isResult: Boolean) extends Logging {
  ...
  def this(df: Dataset[_]) = this(df.queryExecution.analyzed, false)
  ...
  val canonicalizedPlan: LogicalPlan = Canonicalizer.execute(logicalPlan)


  val finalPlan: LogicalPlan = {
	  ...
	  val finalPlan = if (isResult) Project(aliasedOutput, SubqueryAlias(finalName, canonicalizedPlan)) else canonicalizedPlan
	  finalPlan
  }

  val replacedPlan: LogicalPlan = {

	  val replaced = finalPlan.transformAllExpressions {
		  case s: SubqueryExpression =>
			  val query = new SQLBuilder(s.plan, false).toSQL
			  val sql = s match {
  ...
  private def toSQL(node: LogicalPlan): String = node match {
    ...
    case p: Union  =>
	    val children = p.children.filter {
		    case l: LocalRelation =>
			    println(l.output.foreach(print))
			    l.data.nonEmpty
	        case _ => true
	    }
	    val childrenSql = children.map(c => s"(${toSQL(c)})")
    ...
    case v: View => build(toSQL(v.child))
    ...
    case SQLTable(database, table, _, sample) =>
      val qualifiedName = s"${quoteIdentifier(table)}"
    ...
    case OneRowRelation =>
      ""
   /* case l@ LogicalRelation(relation, attr, Some(catalogTable)) =>
	    s" ${catalogTable.identifier.quotedString}"*/

    case p@PushDownReplaced(name, output) =>
	    s"$name"
	    /*build(
		    "SELECT",
		    p.output.map(_.sql).mkString(", "),
		    "FROM",
		    name
	    )*/
    case l@LocalRelation(output, data) => ""
    case _ =>
  ...
  private def aggregateToSQL(plan: Aggregate): String = {
	...
	val aggregateSQL = if (plan.aggregateExpressions.nonEmpty) plan.aggregateExpressions.map(_.sql).mkString(", ")
	else if (plan.groupingExpressions.nonEmpty) groupingSQL
  ...
  object Canonicalizer extends RuleExecutor[LogicalPlan] {
	...
	object AddProject extends Rule[LogicalPlan] {
	  override def apply(tree: LogicalPlan): LogicalPlan = {
		  val newTree = tree match {
			  ...
			  case s: Sort => s
			  case _ => Project(tree.output, tree)
		  }

		  newTree transformUp {
			  case j: Join => j.copy(left = addProjectIfNeeded(j.left), right = addProjectIfNeeded(j.right))
			  case s: Sort => s.copy(child = addProjectIfNeeded(s.child))
			  case l: LocalLimit => l.copy(child = addProjectIfNeeded(l.child))
			  case r: PushDownReplaced => addProject(r)
		  }
	  }
		private def addProjectIfNeeded(plan: LogicalPlan): LogicalPlan = plan match {
			case _: Filter => addProject(plan)
			case j: Join => addProject(j)
			case _ => plan
		}

		private def addProject(plan: LogicalPlan): LogicalPlan = {
			Project(plan.output, plan)
		}
    }
    ...
    object AddSubquery extends Rule[LogicalPlan] {
      override def apply(tree: LogicalPlan): LogicalPlan = tree transformUp {
        ...
        //case p @ Project(_, f @ Filter(_, _: Window)) => p.copy(child  = addSubquery(f))

        case w @ Window(_, _, _, f @ Filter(_, _: Aggregate)) => w.copy(child = addSubquery(f))

        //case w @ Window(_, _, _, f @ Filter(_, _: Window)) => w.copy(child = addSubquery(f))
    ...
    object ConstructSubqueryExpressions extends Rule[LogicalPlan] {
      def apply(tree: LogicalPlan): LogicalPlan = tree transformAllExpressions {
        case ScalarSubquery(query, conditions, exprId) if conditions.nonEmpty =>
	      ...
	      // 0828 15:27
        case ScalarSubquery(query, conditions, exprId) if conditions.isEmpty =>
	        val cleaned = query match {
		        case agg@Aggregate(_, _, Project(projectList, _child)) if projectList.isEmpty =>
					agg.copy(child = _child)
		        case child => child
	        }
	        ScalarSubquery(cleaned, Seq.empty, exprId)
        ...
        case PredicateSubquery(query, conditions, true, exprId) =>
          ...
          In(value, Seq(ListQuery(Project(inner, filtered), Seq.empty, exprId)))*/
      }
    }
  ...
  object ExtractSQLTable {
    def unapply(plan: LogicalPlan): Option[SQLTable] = plan match {
      case l @ LogicalRelation(_, _, Some(catalogTable)) =>
	      //val database = catalogTable.identifier.database.get
	      //val table = catalogTable.identifier.table
	      val database = l.catalogTable.get.identifier.database.get
	      val table  = l.catalogTable.get.identifier.table
