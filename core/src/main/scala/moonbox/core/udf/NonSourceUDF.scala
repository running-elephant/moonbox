...
object NonSourceUDF {
	def apply(className: String, methodName: Option[String]): (AnyRef, DataType) = {
		val (argumentNum, returnType) = getFunctionReturnType(className, methodName)
		(generateFunction(className, methodName, argumentNum), returnType)
	}
	...
	private def getFunctionReturnType(className: String, methodName: Option[String]): (Int, DataType) = {
		...
		val method = try {
			...
			case e: Exception =>
				UdfUtils.getMethod(clazz, methodName.getOrElse("apply"))
		}
