...
class MoonboxAstBuilder extends MqlBaseBaseVisitor[AnyRef] {
  ...
  override def visitRemoveOrganizationOptions(ctx: RemoveOrganizationOptionsContext): MbCommand = {
    val name = ctx.name.getText
    val keys = visitPropertyKeyList(ctx.propertyKeyList())
    AlterOrganizationRemoveOptions(name, keys)
  }
  ...
  override def visitRemoveSaOptions(ctx: RemoveSaOptionsContext): MbCommand = {
    val name = ctx.name.getText
    val organization = ctx.org.getText
    val keys = visitPropertyKeyList(ctx.propertyKeyList())
    AlterSaRemoveOptions(name, keys, organization)
  }
  ...
  override def visitRemoveUserOptions(ctx: RemoveUserOptionsContext): MbCommand = {
    val name = ctx.name.getText
    val keys = visitPropertyKeyList(ctx.propertyKeyList())
    AlterUserRemoveOptions(name, keys)
  }
  ...
  override def visitRemoveDatabaseProperties(ctx: RemoveDatabasePropertiesContext): MbCommand = {
    val name = ctx.name.getText
    val keys = visitPropertyKeyList(ctx.propertyKeyList())
    AlterDatabaseRemoveOptions(name, keys)
  }
  ...
  override def visitPropertyKeyList(ctx: PropertyKeyListContext): Seq[String] = {
    ctx.propertyKey().map { prop =>
      visitPropertyKey(prop)
    }
  }
  ...
  override def visitRemoveTableProperties(ctx: RemoveTablePropertiesContext): MbCommand = {
    val tableIdentifier = visitTableIdentifier(ctx.name)
    val keys = visitPropertyKeyList(ctx.propertyKeyList())
    AlterTableRemoveOptions(tableIdentifier, keys)
  }
