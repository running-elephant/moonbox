...
object EdpLogging {
	...
	try {
		...
		val bridgeClass = Util.classForName("org.slf4j.bridge.SLF4JBridgeHandler")
	...
	private def initializeLogging(): Unit = {
		...
		if (usingLog4j12) {
			...
			if (!log4j12Initialized) {
				val defaultLogProps = "edp/moonbox/common/log4j-defaults.properties"
				Option(Option(Thread.currentThread().getContextClassLoader).getOrElse(getClass.getClassLoader).getResource(defaultLogProps)) match {
					case Some(url) =>
