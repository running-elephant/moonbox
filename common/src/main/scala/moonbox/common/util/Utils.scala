...
object Utils extends MbLogging {
  ...
  def getDefaultYarnPropertyFiles(env: Map[String, String] = sys.env): Seq[String] = {
    val configDir: Option[String] = env.get("YARN_CONF_DIR")
    if (configDir.isEmpty) {
      logWarning("YARN_CONF_DIR config doesn't exist.")
    }
    val configFiles = configDir.map(dir => new File(dir))
      .filter(_.isDirectory)
      .map { dir =>
        dir.listFiles()
          .filter(file => file.isFile && file.getAbsolutePath.endsWith(".xml"))
          .map(_.getAbsolutePath)
      }
    configFiles.getOrElse(Array.empty[String]).toSeq
  }
