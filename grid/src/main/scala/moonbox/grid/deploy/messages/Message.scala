...
object Message {
  ...
  case class SchemaRequest(org: String, username: String, sqls: Seq[String], database: Option[String], analyzable: Option[Boolean]) extends ServiceMessage

  case class SchemaResponses(
                              success: Boolean,
                              message: Option[String] = None,
                              result: Option[Seq[SchemaResponse]] = None) extends ServiceMessage

  sealed trait SchemaResponse extends ServiceMessage

  case class SchemaFailed(message: String) extends SchemaResponse

  case class SchemaSuccessed(`type`: String, fields: Seq[SchemaField]) extends SchemaResponse
