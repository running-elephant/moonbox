...
class MoonboxMasterParam(args: Array[String], val conf: MbConf) extends MbLogging {
	var host = Utils.localHostName()
	var port = 2551

	if (System.getenv("MOONBOX_MASTER_HOST") != null) {
		host = System.getenv("MOONBOX_MASTER_HOST")
	}

	if (System.getenv("MOONBOX_MASTER_PORT") != null) {
		port = System.getenv("MOONBOX_MASTER_PORT").toInt
	}
