...
private[deploy] class MoonboxService(
	...
	private val loginManager = new LoginManager(conf, catalog, this)
	...
	def createApp(user: String, name: String, appType: String, config: Map[String, String]): CreateAppOutbound = {
		askSync[CreateAppResponse](CreateAppRequest(name, appType, config))(SHORT_TIMEOUT) match {
			case Left(CreateAppResponse(success, message)) =>
				CreateAppOutbound(success, message)
			case Right(message) =>
				CreateAppOutbound(success = false, message = Some(message))
		}
	}

	def updateApp(user: String, name: String, config: Map[String, String]): UpdateAppOutbound = {
		askSync[UpdateAppResponse](UpdateAppRequest(name, config))(SHORT_TIMEOUT) match {
			case Left(UpdateAppResponse(success, message)) =>
				UpdateAppOutbound(success, message)
			case Right(message) =>
				UpdateAppOutbound(success = false, Some(message))
		}
	}

	def deleteApp(user: String, name: String): DeleteAppOutbound = {
		askSync[DeleteAppResponse](DeleteAppRequest(name))(SHORT_TIMEOUT) match {
			case Left(DeleteAppResponse(success, message)) =>
				DeleteAppOutbound(success, message)
			case Right(message) =>
				DeleteAppOutbound(success = false, Some(message))
		}
	}

	def startApp(name: String, worker: Option[String] = None): StartAppOutbound = {
		askSync[StartAppResponse](StartAppRequest(name, worker))(SHORT_TIMEOUT * 10) match {
			case Left(StartAppResponse(success, message)) =>
				StartAppOutbound(success, message)
			case Right(message) =>
				StartAppOutbound(success = false, Some(message))
		}
	}

	def stopApp(name: String): StopAppOutbound = {
		askSync[StopAppResponse](StopAppRequest(name))(SHORT_TIMEOUT) match {
			case Left(StopAppResponse(success, message)) =>
				StopAppOutbound(success, message)
			case Right(message) =>
				StopAppOutbound(success = false, Some(message))
		}
	}
