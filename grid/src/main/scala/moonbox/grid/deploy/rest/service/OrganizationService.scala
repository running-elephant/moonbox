...
class OrganizationService(catalog: JdbcCatalog) extends SessionConverter with MbLogging {
	...
	def deleteOrganizations(orgs: Seq[String])(implicit user: User): Future[Either[Unit, Throwable]] = Future {
		try {
			orgs.foreach(org => catalog.dropOrganization(org, ignoreIfNotExists = true, cascade = true))
			Left(Unit)
		} catch {
			case e: Throwable => Right(e)
		}
